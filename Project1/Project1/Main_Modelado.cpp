...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
	    ...
	    model = glm::mat4(1.0f);
		model = glm::scale(model,glm::vec3(3.0f,0.1f,2.0f));//en esta parte del codigo loq uer damos es el ancho, el grosor y la profundidad 
		model = glm::translate(model, glm::vec3(0.0f, 0.9f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		//primera pata de la mesa 
		model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(0.1f, 0.6f, 0.1f));//dibujamos la pata de la mesa 
